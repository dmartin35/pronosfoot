...
def check_forecasts():
    ...
    title = 'Rappel'
    subtitle_tpl = '%(week_loc)s journee'

    #load plain text mail template
    reminder_tpl = get_template('reminder.txt')
    ...
    for week in list(weeks.keys()):
        ...
        for user in mailinglist:
            ...
            if number_matches > 0:
                ...
                subject = '%(title)s - %(subtitle)s' % {
                    'title': title,
                    'subtitle': subtitle_week,
                }
                send_mail_plain_text(subject, email, reminder_content)